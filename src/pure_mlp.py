...
# 训练参数
TRAIN_RATIO = "ratio_50"
TRAIN_STEPS = 100000  # 同样跑10万步，公平对比
LR_INIT = 1e-3
WEIGHT_Lb = 1  # 纯MLP只用这个
GRAD_CLIP = 1.0

# 断点续训参数（🔥 独立MLP路径）
SAVE_STEP = 10000
CHECKPOINT_PATH = f"fd_mlp_{TRAIN_RATIO}_checkpoint"
RESUME_TRAINING = False  # 纯MLP建议从头跑


# ==========================
# 3. MLP 网络结构（和PINN完全一致）
# ==========================
...
def train_model():
    ...
    print(f"\n" + "=" * 50)
    print(f"🚀 开始【纯MLP对比实验】（无物理约束）")
    print(f"=" * 50)
    print(f"总步数: {TRAIN_STEPS} | 初始学习率: {LR_INIT}")
    print(f"当前工况: {TRAIN_RATIO} 测点")
    ...
    for step in range(start_step, TRAIN_STEPS):
        ...
        if step % 1000 == 0:
            ...
            print(
                f"Step [{step:6d}/{TRAIN_STEPS}] | 总损失: {LT.item():.6f} | Lb: {Lb.item():.6f}")
            print(
    ...
    # 🔥 保存文件加 mlp_only 后缀
    torch.save(model.state_dict(), f"fd_mlp_{TRAIN_RATIO}_final.pth")
    np.save(f"train_history_mlp_{TRAIN_RATIO}_final.npy", history)
    print(f"✅ 【纯MLP】训练完成！最终模型已保存为 fd_mlp_{TRAIN_RATIO}_final.pth")

    final_r2, final_rmse = evaluate_model(model, data_dict)
    print(f"\n===== 【纯MLP】最终验证集精度 =====")
    print(f"R²: {final_r2:.4f} | RMSE: {final_rmse:.4f} m/s")
